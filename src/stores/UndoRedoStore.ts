...
const useUndoRedoStore = create<UndoRedoState>((set, get) => ({
  locked: false,
  undoStack: [],
  redoStack: [],
  ...
  undo: async () => {
    ...
    if (locked || undoStack.length === 0) throw new Error("undo invoked when existing operation in progress or empty stack");
    set({ locked: true });
    const [doCb, undoCb] = undoStack[0];
    set({ undoStack: undoStack.slice(1), redoStack: [doCb, ...redoStack] });
    await undoCb();
    set({ locked: false });
  },
  ...
  redo: async () => {
    ...
    if (locked || redoStack.length === 0) throw new Error("redo invoked when existing operation in progress or empty stack");
